...
type
  ...
  public
    ...
    function calPuntos(xn:Real;expre:string):Real;
    constructor Create;
...
function TCustParse.calPuntos(xn:Real;expre:string):Real;
var FParserPunt:TFPExpressionParser;
begin
     FParserPunt:=TFpExpressionParser.Create(nil);
     try
       FParserPunt.BuiltIns:=[bcMath,bcConversion];//reemplaza el (x^n) por power(x,n) para la grafica
       FParserPunt.Identifiers.AddFunction('power','F','FF',@ExprPower);
       FParserPunt.Identifiers.AddFloatVariable('x', xn);//se usa la variable seteada
       FParserPunt.Expression:=expre;
       Result:=FParserPunt.Evaluate.ResFloat;
       //ecGraf:=Result;
     finally
       FParserPunt.Free;
     end;
